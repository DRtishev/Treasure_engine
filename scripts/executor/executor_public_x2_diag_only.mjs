...
export function stableDiagSummary(rootDir) {
  ...
  const hosts = [...new Set(matrix.map((x) => x.endpoint))].sort();
  ...
  const payload = JSON.stringify({ provider_order, hosts, net_family: Number(j.net_family || 0), root_cause_code: String(j.root_cause_code || 'NONE'), smoke_summary });
  const digest = crypto.createHash('sha256').update(payload).digest('hex');
  return { ok: true, root_cause_code: String(j.root_cause_code || 'NONE'), provider_order, matrix, net_family: Number(j.net_family || 0), hosts, digest };
}
