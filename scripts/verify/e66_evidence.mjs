...
const required = ['VERDICT.md', 'PACK.md', 'DIFFS.md', 'PROVENANCE.md', 'RUNS.md', 'CAS.md'];
...
const evidenceFiles = fs.readdirSync(E66_ROOT)
  .filter((f) => f.endsWith('.md') && f !== 'SHA256SUMS.md')
  .sort();
const lines = evidenceFiles.map((f) => `${sha256File(path.join(E66_ROOT, f))}  reports/evidence/E66/${f}`);
