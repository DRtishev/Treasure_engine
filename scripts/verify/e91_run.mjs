...
import { E91_ROOT, ensureDir, isQuiet, minimalLog } from './e91_lib.mjs';
...
for(const k of Object.keys(process.env)) if(process.env.CI==='true'&&/^(UPDATE_E91_|UPDATE_|APPLY_|ENABLE_DEMO_ADAPTER|ALLOW_MANUAL_RECON|ALLOW_NETWORK|DEMO_|DRILL_|CLEAR_)/.test(k)&&String(process.env[k]||'').trim()!=='') throw new Error(`${k} forbidden when CI=true`);
function run(name,cmd,env){const r=spawnSync(cmd[0],cmd.slice(1),{stdio:'inherit',env});if((r.status??1)!==0) throw new Error(`verify:e91 failed at ${name}`);} 
...
const pre=['# E91 PREFLIGHT',`- pwd: ${process.cwd()}`,`- branch: ${spawnSync('git',['branch','--show-current'],{encoding:'utf8'}).stdout.trim()}`,`- head: ${spawnSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).stdout.trim()}`,`- node: ${spawnSync('node',['-v'],{encoding:'utf8'}).stdout.trim()}`,`- npm: ${spawnSync('npm',['-v'],{encoding:'utf8'}).stdout.trim()}`,'- git_status_sb:','```',spawnSync('git',['status','-sb'],{encoding:'utf8'}).stdout.trim(),'```',`- env_CI: ${String(process.env.CI||'')}`,`- env_TZ: ${String(process.env.TZ||'')}`,`- env_LANG: ${String(process.env.LANG||'')}`,`- env_LC_ALL: ${String(process.env.LC_ALL||'')}`].join('\n');
ensureDir(E91_ROOT);if(update&&process.env.CI!=='true') writeMd(path.join(E91_ROOT,'PREFLIGHT.md'),pre);

const before=gitStatus();
if(chainMode==='FULL') run('verify:e90',['npm','run','-s','verify:e90'],{...env,CI:'true'});
else run('verify:e90:pack',['bash','-lc',"grep -E 'canonical_fingerprint' reports/evidence/E90/CLOSEOUT.md reports/evidence/E90/VERDICT.md >/dev/null && grep -E '^[0-9a-f]{64} ' reports/evidence/E90/SHA256SUMS.md | sha256sum -c - >/dev/null"],{...env,CI:'true'});
run('sparsity-suite',['node','scripts/verify/e91_sparsity_suite.mjs'],env);
run('perf-notes',['node','scripts/verify/e91_perf_notes.mjs'],env);
run('no-secrets',['node','scripts/verify/e91_no_secrets_scan.mjs'],env);
run('evidence',['node','scripts/verify/e91_evidence.mjs'],env);
...
minimalLog(`verify:e91 PASSED chain_mode=${chainMode} quiet=${isQuiet()?'1':'0'}`);
