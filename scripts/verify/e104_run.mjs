#!/usr/bin/env node
// E104 Orchestrator - Foundation Adoption + Hardening Quintuple
import fs from 'node:fs';
import path from 'node:path';
import { spawnSync } from 'node:child_process';
import { writeMd } from './e66_lib.mjs';
import { E104_ROOT, ensureDir, isQuiet, minimalLog } from './e104_lib.mjs';
import { isCIMode, forbidEnvInCI, getCIModeString } from './foundation_ci.mjs';
import { repoRootPlaceholder } from './foundation_paths.mjs';
import { gitPresent, gitBranch, gitHead, gitStatusShort, gitStatusPorcelain, getChangedFiles } from './foundation_git.mjs';
import { checkKillLock, enforceNoKillLock, clearKillLock, armKillLock } from './foundation_lock.mjs';

const update = process.env.UPDATE_E104_EVIDENCE === '1';
const chainMode = String(process.env.CHAIN_MODE || (isCIMode() ? 'FAST_PLUS' : 'FAST_PLUS')).toUpperCase();

if (!['FULL', 'FAST_PLUS', 'FAST'].includes(chainMode)) {
  throw new Error('CHAIN_MODE must be FULL|FAST_PLUS|FAST');
}

// E104: CI truthiness enforcement
forbidEnvInCI();

// Kill-lock management
clearKillLock('E104', 'CLEAR_E104_KILL_LOCK');
enforceNoKillLock('E104');

function run(name, cmd, env, critical = true) {
  const r = spawnSync(cmd[0], cmd.slice(1), { stdio: 'inherit', env });
  if ((r.status ?? 1) !== 0) {
    if (critical) armKillLock('E104', `critical_failure:${name}`);
    throw new Error(`verify:e104 failed at ${name}`);
  }
}

const env = {
  ...process.env,
  CHAIN_MODE: chainMode,
  TZ: 'UTC',
  LANG: 'C',
  LC_ALL: 'C',
  SOURCE_DATE_EPOCH: process.env.SOURCE_DATE_EPOCH || '1700000000',
  SEED: String(process.env.SEED || '12345')
};

// PREFLIGHT
const nodeVersion = spawnSync('node', ['-v'], { encoding: 'utf8' }).stdout.trim();
const npmVersion = spawnSync('npm', ['-v'], { encoding: 'utf8' }).stdout.trim();
const osInfo = spawnSync('uname', ['-a'], { encoding: 'utf8' }).stdout.trim();

const pre = [
  '# E104 PREFLIGHT',
  `- pwd: ${repoRootPlaceholder()}`,
  `- branch: ${gitBranch()}`,
  `- head: ${gitHead()}`,
  `- node: ${nodeVersion}`,
  `- npm: ${npmVersion}`,
  `- os: ${osInfo}`,
  `- git_present: ${gitPresent()}`,
  '- git_status_sb:',
  '```',
  gitPresent() ? gitStatusShort() : 'NO_GIT',
  '```',
  `- env_CI: ${getCIModeString()}`
].join('\n');

ensureDir(E104_ROOT);
if (update && !isCIMode()) {
  writeMd(path.join(E104_ROOT, 'PREFLIGHT.md'), pre);
}

const before = gitPresent() ? gitStatusPorcelain() : '';

// Chain to E103 (dependency check)
if (chainMode === 'FULL') {
  run('verify:e103', ['npm', 'run', '-s', 'verify:e103'], { ...env, CI: 'true' });
} else {
  // FAST_PLUS: quick check
  run('verify:e103:pack', ['bash', '-lc',
    "grep -E 'canonical_fingerprint' reports/evidence/E103/CLOSEOUT.md reports/evidence/E103/VERDICT.md >/dev/null"
  ], { ...env, CI: 'true' });
}

// E104 Contracts (execute only if update mode)
if (update && !isCIMode()) {
  // Track B: Porcelain contract
  run('porcelain-contract', ['node', 'scripts/verify/e104_porcelain_contract.mjs'], env, true);

  // Track C: Bundle hash v2 (write mode)
  // Skip in initial run, will be executed by evidence generator

  // Track D: Dep cycle contract
  run('dep-cycle-contract', ['node', 'scripts/verify/e104_dep_cycle_contract.mjs'], env, true);

  // Track E: Speed budget contract (will create baseline if needed)
  // Skip in orchestrator, run separately to avoid circular dependency
}

// Evidence generation
run('evidence', ['node', 'scripts/verify/e104_evidence.mjs'], env);

const after = gitPresent() ? gitStatusPorcelain() : '';

// Scope enforcement
if (before !== after) {
  if (isCIMode()) throw new Error('CI_READ_ONLY_VIOLATION');
  if (!update) throw new Error('READ_ONLY_VIOLATION');

  const changed = getChangedFiles(before, after);
  const allowed = changed.every(r =>
    r.startsWith('reports/evidence/E104/') ||
    r.startsWith('scripts/verify/e104_') ||
    r.startsWith('scripts/verify/foundation_git') ||
    r === '.foundation-seal/E104_KILL_LOCK.md' ||
    r === 'package.json'
  );

  if (!allowed) {
    console.error('Scope violation detected. Changed files:');
    changed.forEach(f => console.error(`  - ${f}`));
    throw new Error('UPDATE_SCOPE_VIOLATION');
  }
}

minimalLog(`verify:e104 PASSED chain_mode=${chainMode} quiet=${isQuiet() ? '1' : '0'} git_present=${gitPresent()}`);
