...
const checks = required.map((r) => {
  ...
  if (!fs.existsSync(abs)) return { path: r, ok: false, status: 'MISSING', detail: 'artifact not found' };
  try {
    const j = JSON.parse(fs.readFileSync(abs, 'utf8'));
    const jStatus = String(j.status || 'UNKNOWN').toUpperCase();
    return { path: r, ok: jStatus === 'PASS', status: jStatus, detail: jStatus === 'PASS' ? 'ok' : 'status must be PASS' };
  } catch {
    return { path: r, ok: false, status: 'INVALID', detail: 'invalid json' };
  }
});

const missing = checks.filter((x) => !x.ok).map((x) => ({ path: x.path, status: x.status, detail: x.detail }));
const frozen = missing.length === 0;
const status = frozen ? 'PASS' : 'BLOCKED';
const reason_code = frozen ? 'NONE' : 'EC01';

writeMd(path.join(EXEC_DIR, 'PROFIT_FOUNDATION_FREEZE_GATE.md'), `# PROFIT_FOUNDATION_FREEZE_GATE.md\n\nSTATUS: ${status}\nREASON_CODE: ${reason_code}\nRUN_ID: ${RUN_ID}\nNEXT_ACTION: ${NEXT_ACTION}\n\n## REQUIRED\n${required.map((x) => `- ${x}`).join('\n')}\n\n## CHECKS\n${checks.map((x) => `- ${x.path}: ${x.status} (${x.detail})`).join('\n')}\n\n## MISSING_OR_BLOCKING\n${missing.map((x) => `- ${x.path}: ${x.status} (${x.detail})`).join('\n') || '- none'}\n\n- foundation_frozen: ${frozen}\n`);
