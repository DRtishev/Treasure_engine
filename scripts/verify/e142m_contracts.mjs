...
const REQUIRED = ['SNAPSHOT.md','DOCTOR_FAST_OUTPUT.md','TRUTH_CACHE_SPEC.md','TRUTH_CACHE_README.md','CAPSULE_INTEGRITY.md','PINNED_NODE_HEALTH.md','NET_CLASSIFICATION.md','BRIDGE_RUN.md','GATE_RUN.md','TRANSFER_EXPORT.md','TRANSFER_IMPORT.md','ACCEPTED.md','CONTRACTS.md','SEAL_X2.md','VERDICT.md','SHA256SUMS.md','RUNBOOK.md'];
...
export function runContracts() {
  ...
  const bridgeOk = fs.existsSync(path.join(FINAL_ROOT, 'BRIDGE_RUN.md')) && /- command: /.test(fs.readFileSync(path.join(FINAL_ROOT, 'BRIDGE_RUN.md'),'utf8')) && /- ec: /.test(fs.readFileSync(path.join(FINAL_ROOT, 'BRIDGE_RUN.md'),'utf8'));
