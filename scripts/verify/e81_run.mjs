...
import { isQuiet, minimalLog } from './e81_lib.mjs';
...
function run(name,cmd,env){const r=spawnSync(cmd[0],cmd.slice(1),{stdio:'inherit',env});if((r.status??1)!==0) throw new Error(`verify:e81 failed at ${name}`);}
...
if(chainMode==='FULL') run('verify:e80',['npm','run','-s','verify:e80'],{...env,CI:'true'});
else run('verify:e80:pack',['bash','-lc',"grep -E 'canonical_fingerprint' reports/evidence/E80/CLOSEOUT.md reports/evidence/E80/VERDICT.md >/dev/null && grep -E '^[0-9a-f]{64} ' reports/evidence/E80/SHA256SUMS.md | sha256sum -c - >/dev/null"],{...env,CI:'true'});
run('verify:wow',['npm','run','-s','verify:wow'],env);
run('verify:wow:usage',['npm','run','-s','verify:wow:usage'],{...env,WOW_USED:'W-0003,W-0019,W-0020'});
run('recon',['node','scripts/verify/e81_recon_observed_multi.mjs'],env);
run('calibration',['node','scripts/verify/e81_calibration_court.mjs'],env);
run('canary-x2',['node','scripts/verify/e81_edge_canary_x2.mjs'],env);
run('evidence',['node','scripts/verify/e81_evidence.mjs'],env);
...
minimalLog(`verify:e81 PASSED chain_mode=${chainMode} quiet=${isQuiet()?'1':'0'}`);
