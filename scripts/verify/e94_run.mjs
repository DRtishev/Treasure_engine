...
import { E94_ROOT, ensureDir, isQuiet, minimalLog } from './e94_lib.mjs';
...
function run(name,cmd,env){const r=spawnSync(cmd[0],cmd.slice(1),{stdio:'inherit',env});if((r.status??1)!==0) throw new Error(`verify:e94 failed at ${name}`);} 
...
const pre=['# E94 PREFLIGHT',`- pwd: ${process.cwd()}`,`- branch: ${spawnSync('git',['branch','--show-current'],{encoding:'utf8'}).stdout.trim()}`,`- head: ${spawnSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).stdout.trim()}`,`- node: ${spawnSync('node',['-v'],{encoding:'utf8'}).stdout.trim()}`,`- npm: ${spawnSync('npm',['-v'],{encoding:'utf8'}).stdout.trim()}`,'- git_status_sb:','```',spawnSync('git',['status','-sb'],{encoding:'utf8'}).stdout.trim(),'```',`- env_CI: ${String(process.env.CI||'')}`,`- env_TZ: ${String(process.env.TZ||'')}`,`- env_LANG: ${String(process.env.LANG||'')}`,`- env_LC_ALL: ${String(process.env.LC_ALL||'')}`].join('\n');
ensureDir(E94_ROOT);if(update&&process.env.CI!=='true') writeMd(path.join(E94_ROOT,'PREFLIGHT.md'),pre);

const before=gitStatus();
if(chainMode==='FULL') run('verify:e93',['npm','run','-s','verify:e93'],{...env,CI:'true'});
else run('verify:e93:pack',['bash','-lc',"grep -E 'canonical_fingerprint' reports/evidence/E93/CLOSEOUT.md reports/evidence/E93/VERDICT.md >/dev/null && grep -E '^[0-9a-f]{64} ' reports/evidence/E93/SHA256SUMS.md | sha256sum -c - >/dev/null"],{...env,CI:'true'});
run('cadence-update',['node','scripts/verify/e94_cadence_update.mjs'],env);
run('ordering-contract',['node','scripts/verify/e94_ordering_contract.mjs'],env);
run('promotion-court',['node','scripts/verify/e94_promotion_multiwindow_court.mjs'],env);
run('perf-notes',['node','scripts/verify/e94_perf_notes.mjs'],env);
run('no-secrets',['node','scripts/verify/e94_no_secrets_scan.mjs'],env);
run('evidence',['node','scripts/verify/e94_evidence.mjs'],env);
...
minimalLog(`verify:e94 PASSED chain_mode=${chainMode} quiet=${isQuiet()?'1':'0'}`);
