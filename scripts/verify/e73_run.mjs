...
if (!['FULL', 'FAST_PLUS', 'FAST'].includes(chainMode)) { console.error('verify:e73 FAILED\n- CHAIN_MODE must be FULL|FAST_PLUS|FAST'); process.exit(1); }
if (process.env.CI === 'true' && update) { console.error('verify:e73 FAILED\n- UPDATE_E73_EVIDENCE=1 forbidden when CI=true'); process.exit(1); }
for (const k of Object.keys(process.env)) {
  if ((k.startsWith('UPDATE_') || k.startsWith('APPROVE_')) && process.env.CI === 'true' && String(process.env[k] || '').trim() !== '') {
    console.error(`verify:e73 FAILED\n- ${k} forbidden when CI=true`); process.exit(1);
  }
}
...
function runStep(name, cmd, env) { const r = spawnSync(cmd[0], cmd.slice(1), { stdio: 'inherit', env }); if ((r.status ?? 1) !== 0) { console.error(`verify:e73 FAILED at ${name}`); process.exit(1); } }
...
if (chainMode === 'FULL') {
  for (const s of ['verify:e66', 'verify:phoenix:x2', 'verify:evidence', 'verify:e67', 'verify:e68', 'verify:e69', 'verify:e70', 'verify:e71', 'verify:e72']) runStep(s, ['npm', 'run', '-s', s], ciEnv);
} else {
  for (const s of ['verify:evidence', 'verify:e67:evidence', 'verify:e68:evidence', 'verify:e69:evidence', 'verify:e70:evidence', 'verify:e71:evidence']) runStep(s, ['npm', 'run', '-s', s], { ...ciEnv, CHAIN_MODE: 'FAST' });
  runStep('verify:e72:pack-static', ['bash','-lc',"grep -E 'canonical_fingerprint' reports/evidence/E72/CLOSEOUT.md reports/evidence/E72/VERDICT.md >/dev/null && grep -E '^[0-9a-f]{64} ' reports/evidence/E72/SHA256SUMS.md | sha256sum -c - >/dev/null"], { ...ciEnv, CHAIN_MODE: 'FAST' });
}
runStep('verify:wow', ['npm', 'run', '-s', 'verify:wow'], env);
runStep('verify:wow:usage', ['npm', 'run', '-s', 'verify:wow:usage'], env);
runStep('verify:contract:court', ['npm', 'run', '-s', 'verify:contract:court'], env);
runStep('verify:edge:contract:x2', ['npm', 'run', '-s', 'verify:edge:contract:x2'], env);
runStep('verify:e73:evidence', ['npm', 'run', '-s', 'verify:e73:evidence'], env);

const after = gitStatus();
if (before !== after) {
  if (process.env.CI === 'true') { console.error('verify:e73 FAILED\n- CI_READ_ONLY_VIOLATION'); process.exit(1); }
  if (!update) { console.error('verify:e73 FAILED\n- READ_ONLY_VIOLATION'); process.exit(1); }
  if (!driftOnlyAllowed(before, after)) { console.error('verify:e73 FAILED\n- UPDATE_SCOPE_VIOLATION'); process.exit(1); }
}
console.log(`verify:e73 PASSED chain_mode=${chainMode}`);
