...
if(!fs.existsSync(E99_ROOT)){
  console.log('e99:no_secrets_scan SKIP (no evidence yet)');
  process.exit(0);
}
...
for(const f of mdFiles){
  ...
  for(let i=0;i<lines.length;i++){
    const line=lines[i];
    // Skip lines that are just file paths (git status output)
    if(/^[?]{2}\s+/.test(line)||/^\s*[MAD]\s+/.test(line)) continue;
    // Skip lines that are reporting on security checks themselves
...
if(violations.length>0){
  console.error(`E99 no-secrets scan FAILED: ${violations.length} potential secrets`);
...
console.log('e99:no_secrets_scan PASSED');
