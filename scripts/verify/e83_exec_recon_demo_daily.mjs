...
import { E83_ROOT, ensureDir, minimalLog } from './e83_lib.mjs';
...
function fetchSymbolWindow(symbol,windowId){
  const host=process.env.DEMO_HOST||'postman-echo.com';
  const reqPath=(process.env.DEMO_PATH_TEMPLATE||'/get?symbol={symbol}&window={window}&limit=1').replace('{symbol}',encodeURIComponent(symbol)).replace('{window}',String(windowId));
  return new Promise((resolve,reject)=>{
    const req=https.request({host,path:reqPath,method:'GET',headers:{'x-demo-key':process.env.DEMO_API_KEY,'x-demo-secret':process.env.DEMO_API_SECRET}},(res)=>{let body='';res.on('data',(c)=>body+=c);res.on('end',()=>{if(res.statusCode<200||res.statusCode>=300) return reject(new Error(`demo_http_${symbol}_${windowId}_${res.statusCode}`));try{resolve({symbol,windowId,host,reqPath,payload:JSON.parse(body)});}catch{reject(new Error(`demo_json_parse_fail_${symbol}_${windowId}`));}});});
...
for(const symbol of symbols) for(let windowId=1;windowId<=windows;windowId++){
  const {host,reqPath,payload}=await fetchSymbolWindow(symbol,windowId);
  ...
  rows.push({symbol,windowId,arg_keys:Object.keys(payload.args||{}).sort().join(','),header_count:Object.keys(payload.headers||{}).length,url_len:String(payload.url||'').length,payloadHash,host,path:reqPath});
}
...
const md=['# E83 EXEC RECON DEMO DAILY',`- symbols: ${symbols.join(',')}`,`- windows_per_symbol: ${windows}`,`- daily_recon_fingerprint: ${fp}`,'','| symbol | window | arg_keys | header_count | url_len | payload_hash_sha256 |','|---|---:|---|---:|---:|---|',...rows.map((r)=>`| ${r.symbol} | ${r.windowId} | ${r.arg_keys} | ${r.header_count} | ${r.url_len} | ${r.payloadHash} |`)].join('\n');
...
ensureDir(E83_ROOT);writeMd(path.join(E83_ROOT,'EXEC_RECON_DEMO_DAILY.md'),md);
minimalLog(`verify:exec:recon:demo:daily PASSED daily_recon_fingerprint=${fp}`);
