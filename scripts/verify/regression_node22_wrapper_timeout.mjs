...
const hasExecForwarding = /bash -lc "source '\$NVM_SH' && nvm use 22\.22\.0 >\/dev\/null && exec \\\"\\\$@\\\"" bash "\$@"/.test(src);
const hasTimeoutMapping = /if \[\[ \$rc -eq 124 \]\]; then\n\s+emit_gate "wrapped_cmd" "timeout" "ENV02"/.test(src);

const status = hasTimeout && hasExecForwarding && hasTimeoutMapping ? 'PASS' : 'BLOCKED';
