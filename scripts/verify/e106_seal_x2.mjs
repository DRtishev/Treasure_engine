#!/usr/bin/env node
// E106 Seal X2 - Meta-determinism proof
// Regenerates E106 evidence twice and proves identity
...
const E106_ROOT = path.resolve('reports/evidence/E106');
const SEAL_PATH = path.join(E106_ROOT, 'SEAL_X2.md');

console.log('e106:seal_x2: Running first evidence generation...');

// First seal
const result1 = spawnSync('npm', ['run', '-s', 'verify:e106:update'], {
  stdio: 'pipe',
  env: { ...process.env, CI: 'false', UPDATE_E106_EVIDENCE: '1', CHAIN_MODE: 'FAST_PLUS', QUIET: '1' }
});
...
const seal1 = {
  closeout: sha256File(path.join(E106_ROOT, 'CLOSEOUT.md')),
  verdict: sha256File(path.join(E106_ROOT, 'VERDICT.md')),
  sums: sha256File(path.join(E106_ROOT, 'SHA256SUMS.md'))
};

console.log('e106:seal_x2: First seal captured');
...
console.log('e106:seal_x2: Running second evidence generation...');

// Second seal
const result2 = spawnSync('npm', ['run', '-s', 'verify:e106:update'], {
  stdio: 'pipe',
  env: { ...process.env, CI: 'false', UPDATE_E106_EVIDENCE: '1', CHAIN_MODE: 'FAST_PLUS', QUIET: '1' }
});
...
console.log('e106:seal_x2: Second seal captured');
...
console.log('e106:seal_x2: Comparison');
...
if (!closeoutMatch || !verdictMatch || !sumsMatch) {
  console.error('e106:seal_x2 FAILED - Evidence generation is not deterministic');
  process.exit(1);
}

console.log('e106:seal_x2 PASSED');
