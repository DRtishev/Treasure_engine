...
import { isQuiet, minimalLog } from './e80_lib.mjs';
...
function run(name,cmd,env){const r=spawnSync(cmd[0],cmd.slice(1),{stdio:'inherit',env});if((r.status??1)!==0) throw new Error(`verify:e80 failed at ${name}`);}
...
if(chainMode==='FULL') run('verify:e79',['npm','run','-s','verify:e79'],{...env,CI:'true'});
else run('verify:e79:pack',['bash','-lc',"grep -E 'canonical_fingerprint' reports/evidence/E79/CLOSEOUT.md reports/evidence/E79/VERDICT.md >/dev/null && grep -E '^[0-9a-f]{64} ' reports/evidence/E79/SHA256SUMS.md | sha256sum -c - >/dev/null"],{...env,CI:'true'});
run('verify:wow',['npm','run','-s','verify:wow'],env);
run('verify:wow:usage',['npm','run','-s','verify:wow:usage'],{...env,WOW_USED:'W-0003,W-0019,W-0020'});
run('recon',['node','scripts/verify/e80_recon_observed_expand.mjs'],env);
run('calibration',['node','scripts/verify/e80_calibration_court.mjs'],env);
run('shortlist',['node','scripts/verify/e80_edge_shortlist.mjs'],env);
run('canary-x2',['node','scripts/verify/e80_edge_canary_x2.mjs'],env);
run('evidence',['node','scripts/verify/e80_evidence.mjs'],env);
...
minimalLog(`verify:e80 PASSED chain_mode=${chainMode} quiet=${isQuiet()?'1':'0'}`);
