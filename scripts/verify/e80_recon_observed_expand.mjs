...
const coverageFingerprint=crypto.createHash('sha256').update(JSON.stringify({recon_fingerprint:recon.fingerprint,summary})).digest('hex');

if(update&&process.env.CI!=='true'){ensureDir(E80_ROOT);const lines=['# E80 EXEC RECON OBSERVED MULTI',`- source_file: ${recon.source_file}`,`- source_sha256: ${recon.source_sha}`,`- recon_fingerprint: ${recon.fingerprint}`,`- coverage_report_fingerprint: ${coverageFingerprint}`,'','| symbol | windows_count | invalid_row_rate | drift_proxy | spread_p50 | fee_avg |','|---|---:|---:|---:|---:|---:|'];for(const s of summary) lines.push(`| ${s.symbol} | ${s.windows_count} | ${s.invalid_row_rate.toFixed(8)} | ${s.drift_proxy.toFixed(8)} | ${s.spread_p50.toFixed(6)} | ${s.fee_avg.toFixed(6)} |`);writeMd(path.join(E80_ROOT,'EXEC_RECON_OBSERVED_MULTI.md'),lines.join('\n'));}
quietLog(JSON.stringify({recon_fingerprint:recon.fingerprint,coverage_report_fingerprint:coverageFingerprint},null,2));
minimalLog(`verify:e80:recon PASSED coverage_report_fingerprint=${coverageFingerprint}`);
