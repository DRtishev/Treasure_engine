...
if (precheckPath && fs.existsSync(precheckPath)) {
  ...
  offenders = out.filter((p) => String(p).startsWith('reports/evidence/EXECUTOR/')).sort((a,b)=>a.localeCompare(b));
}

const ok = offenders.length === 0;
const status = ok ? 'PASS' : 'BLOCKED';
const reason = ok ? 'NONE' : 'RG_EXEC01';
writeMd(path.join(EXEC, 'REGRESSION_EXEC01_NO_EXECUTOR_WRITES_DURING_VICTORY.md'), `# REGRESSION_EXEC01_NO_EXECUTOR_WRITES_DURING_VICTORY.md\n\nSTATUS: ${status}\nREASON_CODE: ${reason}\nRUN_ID: ${RUN_ID}\nNEXT_ACTION: npm run -s epoch:victory:seal\n\n${offenders.length ? offenders.map((o)=>`- offender: ${o}`).join('\n') : '- offenders: []'}\n`);
writeJsonDeterministic(path.join(MANUAL, 'regression_exec01_no_executor_writes_during_victory.json'), { schema_version:'1.0.0', status, reason_code: reason, run_id: RUN_ID, offenders });
console.log(`[${status}] regression_exec01_no_executor_writes_during_victory — ${reason}`);
process.exit(ok ? 0 : 2);
