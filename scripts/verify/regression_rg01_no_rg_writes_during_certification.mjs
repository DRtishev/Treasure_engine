...
if (precheckPath && fs.existsSync(precheckPath)) {
  ...
  offenders = out.filter((p) => String(p).startsWith('reports/evidence/RG_')).sort((a,b)=>a.localeCompare(b));
}

const ok = offenders.length === 0;
const status = ok ? 'PASS' : 'BLOCKED';
const reason = ok ? 'NONE' : 'RG_RG01';
writeMd(path.join(EXEC, 'REGRESSION_RG01_NO_RG_WRITES_DURING_CERTIFICATION.md'), `# REGRESSION_RG01_NO_RG_WRITES_DURING_CERTIFICATION.md\n\nSTATUS: ${status}\nREASON_CODE: ${reason}\nRUN_ID: ${RUN_ID}\nNEXT_ACTION: npm run -s epoch:victory:seal\n\n${offenders.length ? offenders.map((o)=>`- offender: ${o}`).join('\n') : '- offenders: []'}\n`);
writeJsonDeterministic(path.join(MANUAL, 'regression_rg01_no_rg_writes_during_certification.json'), { schema_version:'1.0.0', status, reason_code: reason, run_id: RUN_ID, offenders });
console.log(`[${status}] regression_rg01_no_rg_writes_during_certification — ${reason}`);
process.exit(ok ? 0 : 2);
