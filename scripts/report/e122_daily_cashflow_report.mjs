...
import { createLedger, recordFill, getLedgerSummary, detectAnomalies } from '../../core/profit/ledger.mjs';
import { runDirE122, writeMdAtomic } from '../verify/e122_lib.mjs';
...
for (const ev of normalized) {
  ...
  recordFill(ledger, { symbol: ev.symbol, side: ev.side, qty: ev.qty, price: ev.price, exec_price: ev.price, fee: ev.fee, ts: ev.ts, trade_id: ev.trade_id });
}
...
writeMdAtomic('reports/evidence/E122/LEDGER_DAILY_REPORT.md', [
  '# E122 LEDGER DAILY REPORT',
  `- fills: ${ledger.fills.length}`,
  `- fees_usd: ${summary.total_fees.toFixed(4)}`,
  `- realized_pnl_usd: ${summary.realized_pnl.toFixed(4)}`,
  `- drawdown: ${summary.max_drawdown.toFixed(6)}`,
