...
const EPOCH_DIR = path.join(ROOT, 'reports', 'evidence', 'EDGE_PROFIT_00', 'sandbox');
const ROWS = Number(process.env.PAPER_REAL_SANDBOX_ROWS || 360);
const SEED = Number(process.env.PAPER_REAL_SANDBOX_SEED || 20260223);
...
for (let i = 0; i < ROWS; i++) {
  ...
  const sizeRatio = 0.35 + rng() * 1.4;
  const spread = 0.7 + rng() * 2.5;
  const intendedEntry = base + (rng() - 0.5) * (symbol === 'BTCUSDT' ? 60 : 9);
  const slippage = spread / 2 + 1.0 * Math.sqrt(sizeRatio) + (rng() - 0.5) * 0.25;
  const fillPrice = intendedEntry * (1 + (slippage / 10000) * (i % 2 === 0 ? 1 : -1));
  const intendedExit = intendedEntry * (1 + ((rng() - 0.45) / 200));
  const fee = Math.abs(fillPrice) * 0.00003;
  const latency = 50 + rng() * 220;
  const pnl = 0.09 + rng() * 0.16 - (i % 19 === 0 ? 0.07 : 0.0);
  rows.push([
    ts,
    symbol,
    i % 2 === 0 ? 'BUY' : 'SELL',
    `REAL-SANDBOX-${String(i + 1).padStart(5, '0')}`,
...
fs.writeFileSync(PROFILE_PATH, 'sandbox\n');

const csvSha = crypto.createHash('sha256').update(csv).digest('hex');

writeMd(path.join(EPOCH_DIR, 'REAL_SANDBOX_GENERATION.md'), `# REAL_SANDBOX_GENERATION.md\n\nSTATUS: PASS\nREASON_CODE: NONE\nRUN_ID: ${RUN_ID}\nNEXT_ACTION: npm run -s edge:profit:00:import:csv\n\n## Generator\n\n- mode: REAL_SANDBOX\n- rows: ${ROWS}\n- seed: ${SEED}\n- csv_path: artifacts/incoming/raw_paper_telemetry.csv\n- csv_sha256: ${csvSha}\n- profile_marker_path: artifacts/incoming/paper_telemetry.profile\n- profile_marker_value: sandbox\n- source_tag: REAL_SANDBOX_V1\n`);

console.log(`[PASS] paper_telemetry_real_sandbox_gen — rows=${ROWS}`);
