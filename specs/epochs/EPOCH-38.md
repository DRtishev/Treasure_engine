...
## REALITY SNAPSHOT
- E37 is the immediate upstream dependency and this epoch remains specification-only.
- Runtime trading implementation is out of scope for this rewrite cycle.

## GOALS
- Deliver implementation-ready contract language for RealityGapReport without ambiguity.
- Lock deterministic/offline-first gate semantics for future `npm run verify:epoch38` implementation.
...
## DESIGN / CONTRACTS
### Dependencies
- Dependency: `EPOCH-37` as declared in `specs/epochs/INDEX.md` and `specs/epochs/LEDGER.json`.
- Primary contract family: `RealityGapReport`.
- Local canonical example:
```json
{"schema_version":"1.0.0","epoch":38,"contract":"RealityGapReport","seed":12345,"fingerprint":{"algo":"sha256","value":"<hex>"}}
```
...
- Planned future gate command: `npm run verify:epoch38`.
...
## STOP RULES
- BLOCKED if dependency chain (`EPOCH-37` -> `EPOCH-38`) is inconsistent between ledger and index.
...
- WOW hooks:
  - P0: WOW-03 sim-shadow gap quantification — prove via epoch gate report and deterministic replay digest.
  - P1: WOW-19 stress breach auto-brake — prove via contract coverage checklist in evidence summary.
  - P2: WOW-31 gap band explainability — prove via explicit artifact reference in future epoch gate.
