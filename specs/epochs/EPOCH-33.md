# EPOCH-33 — Strategy Registry + StrategySpec Governance

## REALITY SNAPSHOT
- E32 is the immediate upstream dependency and this epoch remains specification-only.
- Runtime trading implementation is out of scope for this rewrite cycle.

## GOALS
- Deliver implementation-ready contract language for StrategySpec without ambiguity.
- Lock deterministic/offline-first gate semantics for future `npm run verify:epoch33` implementation.
...
## DESIGN / CONTRACTS
### Dependencies
- Dependency: `EPOCH-32` as declared in `specs/epochs/INDEX.md` and `specs/epochs/LEDGER.json`.
- Primary contract family: `StrategySpec`.
- Local canonical example:
```json
{"schema_version":"1.0.0","epoch":33,"contract":"StrategySpec","seed":12345,"fingerprint":{"algo":"sha256","value":"<hex>"}}
```
...
- Planned future gate command: `npm run verify:epoch33`.
...
## STOP RULES
- BLOCKED if dependency chain (`EPOCH-32` -> `EPOCH-33`) is inconsistent between ledger and index.
...
- WOW hooks:
  - P0: WOW-07 regime-tagged strategy metadata — prove via epoch gate report and deterministic replay digest.
  - P1: WOW-14 overfit-prevention metadata — prove via contract coverage checklist in evidence summary.
  - P2: WOW-27 immutable promotion manifest — prove via explicit artifact reference in future epoch gate.
