...
## REALITY SNAPSHOT
- E38 is the immediate upstream dependency and this epoch remains specification-only.
- Runtime trading implementation is out of scope for this rewrite cycle.

## GOALS
- Deliver implementation-ready contract language for ShadowEvent + CanaryPhaseState without ambiguity.
- Lock deterministic/offline-first gate semantics for future `npm run verify:epoch39` implementation.
...
## DESIGN / CONTRACTS
### Dependencies
- Dependency: `EPOCH-38` as declared in `specs/epochs/INDEX.md` and `specs/epochs/LEDGER.json`.
- Primary contract family: `ShadowEvent + CanaryPhaseState`.
- Local canonical example:
```json
{"schema_version":"1.0.0","epoch":39,"contract":"ShadowEvent + CanaryPhaseState","seed":12345,"fingerprint":{"algo":"sha256","value":"<hex>"}}
```
...
- Planned future gate command: `npm run verify:epoch39`.
...
## STOP RULES
- BLOCKED if dependency chain (`EPOCH-38` -> `EPOCH-39`) is inconsistent between ledger and index.
...
- WOW hooks:
  - P0: WOW-25 canary ramp governance — prove via epoch gate report and deterministic replay digest.
  - P1: WOW-30 shadow no-order attestation — prove via contract coverage checklist in evidence summary.
  - P2: WOW-34 rollback rehearsal protocol — prove via explicit artifact reference in future epoch gate.
