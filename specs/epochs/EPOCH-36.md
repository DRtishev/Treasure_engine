# EPOCH-36 — Risk Brain FSM + RiskDecision

## REALITY SNAPSHOT
- E35 is the immediate upstream dependency and this epoch remains specification-only.
- Runtime trading implementation is out of scope for this rewrite cycle.

## GOALS
- Deliver implementation-ready contract language for RiskDecision without ambiguity.
- Lock deterministic/offline-first gate semantics for future `npm run verify:epoch36` implementation.
...
## DESIGN / CONTRACTS
### Dependencies
- Dependency: `EPOCH-35` as declared in `specs/epochs/INDEX.md` and `specs/epochs/LEDGER.json`.
- Primary contract family: `RiskDecision`.
- Local canonical example:
```json
{"schema_version":"1.0.0","epoch":36,"contract":"RiskDecision","seed":12345,"fingerprint":{"algo":"sha256","value":"<hex>"}}
```
...
- Planned future gate command: `npm run verify:epoch36`.
...
## STOP RULES
- BLOCKED if dependency chain (`EPOCH-35` -> `EPOCH-36`) is inconsistent between ledger and index.
...
- WOW hooks:
  - P0: WOW-22 anti-blowup controls — prove via epoch gate report and deterministic replay digest.
  - P1: WOW-24 layered loss limits — prove via contract coverage checklist in evidence summary.
  - P2: WOW-29 deterministic kill-switch matrix — prove via explicit artifact reference in future epoch gate.
